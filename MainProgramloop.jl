...
Planeta2 = Dict("M" => 10^17, "coord" => [0,0], "v_vec" => [0,0])
Planeta3 = Dict("M" => 10^16, "coord" => [0,-10000],"v_vec" =>[-20,0])
lista=[Planeta1, Planeta2]
#---------------------------------------------------------------------------------------------
#       FUNKCJE DO OBLICZEŃ 
#---------------------------------------------------------------------------------------------
...
function MainFunction(l,t::Number)
    for i in 1:length(lista)
        FG=0
        for j in 1:length(lista) if j!=i
...
symulation = @animate for i in t
    ...
    markersize = 10)
    scatter!([Planeta3["coord"][1]],[Planeta3["coord"][2]],
    markersize = 5)
    for z in 1:n
